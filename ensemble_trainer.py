...
def main(args):
    ...
    dataset = torchvision.datasets.CIFAR10("/tmp/pycharm_project_840/", train=True, download=True,
    ...
    testset = torchvision.datasets.CIFAR10("/tmp/pycharm_project_840/", train=False, download=True,
    ...
    torch.save(reg_pars, 'reg_params_test2.pth')

    # print("Saving NAMED PARAMETERS")
    # with open('named_parameters.pickle', 'wb') as handle:
    #     pickle.dump(simplex_model.named_parameters(), handle, protocol=pickle.HIGHEST_PROTOCOL)
    # for param in simplex_model.parameters():
    # print(param)
    for name, param in simplex_model.named_parameters():
        print(name)
    # print("Saved NAMED PARAMETERS")
    print(simplex_model.state_dict())
    torch.save(simplex_model.state_dict(), 'state_dict_test2.pth')
    print(utils.eval(testloader, simplex_model, criterion))

    model = SimplexNet(10, VGG16Simplex, n_vert=2)
    model.load_state_dict(torch.load('state_dict_test2.pth'))
    print(utils.eval(testloader, model, criterion))
    print(utils.eval(testloader, simplex_model, criterion))

    print(sum((x - y).abs().sum() for x, y in zip(simplex_model.state_dict().values(), model.state_dict().values())))
