...
class SpiralModel(nn.Module):
    def __init__(self, n_hidden=512, fix_points=[False]):
        super(SpiralModel, self).__init__()
        self.n_hidden = n_hidden
        self.l1 = SimpLinear(2, n_hidden, fix_points=fix_points)
        self.relu = nn.ReLU(inplace=True)
        self.l2 = SimpLinear(n_hidden, n_hidden, fix_points=fix_points)
        self.output1 = SimpLinear(n_hidden, 2, fix_points=fix_points)
        self.logSoftMax = nn.LogSoftmax(dim=1)

    def forward(self, x, coeffs_t):
        x = self.relu(self.l1(x, coeffs_t))
        x = self.relu(self.l2(x, coeffs_t))
        x = self.logSoftMax(self.output1(x, coeffs_t))
